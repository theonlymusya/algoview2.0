...
void parse_value(const std::string& str, std::string& left_value, std::string& right_value) {
    int first_comma_pos = str.find(',');
    char* c_left_value = (char*)malloc(first_comma_pos);
    int left_value_len = str.copy(c_left_value, first_comma_pos, 0);
    c_left_value[left_value_len] = '\0';
    left_value = std::string(c_left_value);
    first_comma_pos++;
    int right_value_len = str.length() - first_comma_pos;
    char* c_right_value = (char*)malloc(right_value_len);
    left_value_len = str.copy(c_right_value, right_value_len, first_comma_pos);
