...
// std::map<std::string, CoordType> params_map;
// std::map<std::string, CoordType> variables_map;
//std::map<std::string, std::map<std::string, std::string>> blocks_variables_map;

// template <typename T>
// const T func(std::string expr) {
...
// 	for(auto it = params_map.cbegin(); it != params_map.cend(); ++it) {
...
//    	symbol_table.add_variable(expr_var,x);
...
class DOM_tree {
	public:
		void parse_xml_file(std::string);
		void print_tree();
		//find out root node
		rapidxml::xml_node<>* get_root_node() { return doc_.first_node("algo"); };
		void print_params_map();
		void print_blocks();
		void handle_tree( DOM_tree& );
		void handle_params();
		const std::map<std::string, CoordType>& get_param_value_map() const
			{ return param_value_map_; }
		class Block {
			public:
				BlockId id = 0;
				Dim dim;
				Block(const BlockId& _id, Dim _dim) : id(_id), dim(_dim) {}
				void handle_arg(DOM_tree&, Block&, const rapidxml::xml_node<> *);
				void print_vars_map();
				const std::map<std::string, CoordType>& get_var_range_map() const { return var_range_map_; }
		...
		Block handle_block(DOM_tree&, const rapidxml::xml_node<> *);
	...
	public:
		...
		class Vertex {
			public:
				...
				Vertex(VertexId _id, VertexType _type = "0") : id(_id), type(_type) {
					coords.resize(3, 0);
				}
		};
		void add_vertex(int);
...
void Graph::vertices_init(const std::vector<DOM_tree::Block>& blocks) {
	for (const auto& block : blocks) {
		...
		if (block.dim == 2) {
			...
			std::cerr << vars_range_map.find("i")->second * vars_range_map.find("j")->second << std::endl;
			add_vertex(vars_range_map.find("i")->second * vars_range_map.find("j")->second);
		}
		if (block.dim == 3) {
			std::cerr << "ya block dim 3" << std::endl;
			std::cerr << vars_range_map.find("i")->second * vars_range_map.find("j")->second * vars_range_map.find("k")->second << std::endl;
			add_vertex(vars_range_map.find("i")->second * vars_range_map.find("j")->second * vars_range_map.find("k")->second);
		}
	}
	std::cerr << "ecgt[";
}

void Graph::offset_coords(int local_offset, int size, int coord, int vert_num) {
//void Graph::offset_coords(int local_offset, int cycle_offset, int coord, int vert_num) {
	...
	// 	if (dim == 3) {
	// 		for (int j = 0; j < cycle_offset; j++) {
	// 			vertices_[local_offset + i + j].coords[coord] += 2*offset;
	// 		}
	// 		i+= cycle_offset;
	// 	}
	// }
...
void Graph::vertices_coords_calc(const std::vector<DOM_tree::Block>& blocks) {
	...
	for (const auto& block : blocks) {
		...
		if (block.dim == 1 || block.dim == 2 || block.dim == 3) {
			offset_coords(vertices_arr_offset, vars_range_map.find("i")->second, X, vertices_in_block_num);
			between_blocks_offset = offset;
		}
		std::cerr << "--------------------------------------------------" << std::endl;
		print_vertices();
		if (block.dim == 2 || block.dim == 3) {
			offset_coords(vertices_arr_offset, vars_range_map.find("j")->second, Y, vertices_in_block_num);
			between_blocks_offset = offset;
		}
		std::cerr << "--------------------------------------------------" << std::endl;
		print_vertices();
		if (block.dim == 3) {
			offset_coords(vertices_arr_offset, vars_range_map.find("k")->second, Z, vertices_in_block_num);
			between_blocks_offset = vars_range_map.find("k")->second * offset;
		}
		std::cerr << "--------------------------------------------------" << std::endl;
		print_vertices();
...
std::string Graph::to_json_format() {
	...
	for (const auto& vertex : vertices_) {
		...
		std::to_string((int)vertex.coords[X]) + ", " + std::to_string((int)vertex.coords[Y]) + ", " +
...
void Graph::print_vertices() {
	for (const auto& vertex : vertices_) {
		std::cout << vertex.id << ". [" << vertex.coords[X] << ", " << vertex.coords[Y] << ", " << vertex.coords[Z] << "]" << std::endl;
	}
}

void DOM_tree::parse_xml_file(std::string filename) {
	std::cout << "Start parsing a test file" << std::endl;
	std::ifstream file ("test.xml"); //open file by using constructor
	std::vector<char> buffer((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
	buffer.push_back('\0');
	doc_.parse<0>(&buffer[0]); // <0> means default parse flags
}
...
void DOM_tree::handle_params() {
	//get parameters names and values
	rapidxml::xml_node<> * params_node = get_root_node()->first_node("params");
	for (rapidxml::xml_node<> * param_node = params_node->first_node("param");
		param_node; param_node = param_node->next_sibling()) {
...
void DOM_tree::Block::print_vars_map() {
	for(auto it = var_range_map_.cbegin(); it != var_range_map_.cend(); ++it)
    	std::cout << it->first << " " << it->second << std::endl;
}
...
void DOM_tree::print_blocks() {
	for (const auto& block : blocks_) {
		...
		const std::map<std::string, CoordType>& var_range_map = block.get_var_range_map();
		for(auto it = var_range_map.cbegin(); it != var_range_map.cend(); ++it)
    		std::cout << it->first << " " << it->second << std::endl;
	}
}

void DOM_tree::Block::handle_arg(DOM_tree& tree, Block& block, const rapidxml::xml_node<> * arg_node) {
	...
	double expr_value = calc_expr<double>(std::string(c_end_value), param_value_map);
   	printf("right barrier %8.2f ", expr_value);
   	std::cout << "var_range " << expr_value - std::stoi(std::string(c_begin_value)) << std::endl;

	block.var_range_map_[var_name] = (int)(expr_value - std::stoi(std::string(c_begin_value)) + 1);	
}

DOM_tree::Block DOM_tree::handle_block(DOM_tree& tree, const rapidxml::xml_node<> * block_node) {
	...
	for (rapidxml::xml_node<> * arg_node = args_node->first_node("arg");
		arg_node; arg_node = arg_node->next_sibling()) {
		block.handle_arg(tree, block, arg_node);
	}
	return block;
}
...
void DOM_tree::handle_tree(DOM_tree& tree) {
	handle_params();
	for (rapidxml::xml_node<> * block_node = get_root_node()->first_node("block");
		block_node; block_node = block_node->next_sibling()) {
		std::string block_id = block_node->first_attribute("id")->value();
		...
		Block block = handle_block(tree, block_node);
